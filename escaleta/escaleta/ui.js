...
// INTERFAZ DE USUARIO PRINCIPAL CON SOPORTE NATIVO DE GEMINI
const EscaletaUI = {
    init() {
    ...
    updateAuthUI(isConnected) {
        ...
        if (ai.aiProvider === 'gemini') {
            if (ai.apiKey) {
                if(text) text.innerText = "GEMINI 3.5 FLASH LITE";
                if(indicator) indicator.className = "w-2 h-2 rounded-full bg-purple-600 shadow-[0_0_8px_rgba(147,51,234,0.6)]";
            } else {
                if(text) text.innerText = "NO GEMINI KEY";
                if(indicator) indicator.className = "w-2 h-2 rounded-full bg-red-400";
            }
            return;
        }
        if (isConnected) {
            if(text) text.innerText = "ONLINE (POLLINATIONS)";
    ...
    toggleConfigModal() {
        ...
        if(modal) {
            ...
            if(!modal.classList.contains('hidden')) {
                const savedGoogleKey = localStorage.getItem('googlecloud_api_key') || "";
                const savedGeminiKey = localStorage.getItem('gemini_api_key') || savedGoogleKey || localStorage.getItem('pollinations_api_key') || "";
                ...
                this.updateProviderFieldsVisibility(ai.aiProvider || 'gemini');
    ...
    updateProviderFieldsVisibility(provider) {
        ...
        if(provider === 'gemini') {
            if(gemBlock) gemBlock.classList.remove('hidden');
            ...
            if(gemBlock) gemBlock.classList.add('hidden');
    ...
    saveSystemConfig() {
        ...
        if (geminiKey) {
            ...
            if (!localStorage.getItem('googlecloud_api_key')) {
