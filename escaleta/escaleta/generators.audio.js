...
// MÓDULO DE GENERACIÓN DE AUDIO (EXTENSIÓN DE GENERATORS) - V5.2 (FILTRO ANTI-ARROBA INTEGRADO)
Object.assign(Generators, {
    ...
    async generateAudioForTake(takeId) {
        ...
        const statusBadge = card ? card.querySelector('.audio-status') : null;

        // Limpieza de seguridad contra arrobas antes de enviar a la síntesis de voz
        const cleanNarration = (take.narration_text || '').replace(/@/g, '').trim();
