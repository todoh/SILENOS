// --- cronologia/ai2.js: ANALISTA ESTRUCTURAL (Escalado Adaptativo Gemini) ---
const ai2 = {
         
  // --- CÁLCULO DE ESTRUCTURA (AUDITORÍA DE DENSIDAD) ---
  async calculateStructure() {
        // 1. Validaciones
        if (!ai.apiKey && ai.aiProvider !== 'ollama') return alert("Configura la API Key de Gemini primero en [CFG].");
        ...
        try {
            ...
            const auditRes = await ai.callModel(auditPrompt, "Audita la historia considerando subpartes densas de forma obligatoria.", 0.2, 'gemini-3.5-flash-lite');
