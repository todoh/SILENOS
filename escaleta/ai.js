// --- cronologia/ai.js ---
const ai = {
    apiKey: localStorage.getItem('koreh_gemini_book_api_key') || localStorage.getItem('gemini_api_key') || localStorage.getItem('googlecloud_api_key') || localStorage.getItem('pollinations_api_key') || '',
    clientId: localStorage.getItem('pollinations_client_id') || '', // ID de cliente global
    ollamaUrl: localStorage.getItem('escaleta_ollama_url') || 'http://127.0.0.1:11434/api',
    aiProvider: localStorage.getItem('escaleta_ai_provider') || 'gemini', // 'gemini' | 'pollinations' | 'ollama'
    geminiModel: localStorage.getItem('escaleta_gemini_model') || 'gemini-3.5-flash-lite',
    pollinationsModel: localStorage.getItem('escaleta_pollinations_model') || 'nova-fast',
    contextHandle: null,
    inventory: [],
    ...
    login() {
        ...
        let authUrl = `https://enter.pollinations.ai/authorize?redirect_url=${redirectUrl}`;
        if (this.clientId) {
            authUrl += `&client_id=${encodeURIComponent(this.clientId)}`;
        }
        window.open(authUrl, 'PollinationsAuth', `width=500,height=700`);
    },

    init() {
        this.apiKey = localStorage.getItem('koreh_gemini_book_api_key') || localStorage.getItem('gemini_api_key') || localStorage.getItem('googlecloud_api_key') || localStorage.getItem('pollinations_api_key') || '';
        this.aiProvider = localStorage.getItem('escaleta_ai_provider') || 'gemini';
        this.geminiModel = localStorage.getItem('escaleta_gemini_model') || 'gemini-3.5-flash-lite';

        // DETECCIÓN DE CONTEXTO DE SUBPROCESO (POPUP POLLINATIONS BACKWARD COMPATIBILITY)
    ...
    // --- API CORE (PROVEEDOR ROUTER CON GEMINI 3.5 FLASH LITE DIRECTO) ---
    async callModel(system, user, temp = 0.7, model = null) {
        this.apiKey = localStorage.getItem('koreh_gemini_book_api_key') || localStorage.getItem('gemini_api_key') || localStorage.getItem('googlecloud_api_key') || localStorage.getItem('pollinations_api_key') || this.apiKey;
        if (this.aiProvider === 'gemini') {
            if (!this.apiKey) {
                throw new Error("Requiere API Key de Gemini / Google Cloud. Configúrala en el panel [CFG].");
            }
            let activeModel = model || this.geminiModel || 'gemini-3.5-flash-lite';
            if (activeModel === 'gemini-fast' || activeModel === 'gemini-flash' || activeModel === 'nova-fast' || activeModel === 'claude-fast') {
        ...
        } else if (this.aiProvider === 'pollinations') {
            ...
            try {
                ...
                if (!response.ok) {
                    if (response.status === 402) {
                        throw new Error("CRÉDITOS INSUFICIENTES (Error 402): Tu saldo de 'Pollen' en Pollinations AI se ha agotado.");
                    }
                    throw new Error(`API Pollinations Error: ${response.status}`);
                }
                const data = await response.json();
    ...
    async generateSmartPremise() {
        if (this.aiProvider !== 'ollama' && !this.apiKey) return alert("Configura la API Key de Gemini primero en [CFG].");
        ...
        try {
            this.log(`Desplegando Analistas Rápidos bajo proveedor: ${this.aiProvider.toUpperCase()} (${this.geminiModel})...`, "brain");
            ...
            const promises = [
                this.callModel(promptPsych, commonContext, 0.5, 'gemini-3.5-flash-lite'),
                this.callModel(promptSocio, commonContext, 0.5, 'gemini-3.5-flash-lite'),
                this.callModel(promptLore, commonContext, 0.5, 'gemini-3.5-flash-lite')
            ];
            ...
            # ESTRUCTURA SUGERIDA`;
            const finalPremise = await this.callModel(directorPrompt, "Sintetiza la historia.", 0.6, 'gemini-3.5-flash-lite');
    ...
    async runPipeline() {
        if (this.aiProvider !== 'ollama' && !this.apiKey) return alert("Configura la API Key de Gemini en [CFG] primero.");
        ...
        try {
            ...
            const phase2Promises = rawChapters.map(async (chapter) => {
                ...
                try {
                    const res = await this.callModel(prompt, "Genera informe adaptado al capítulo con metadatos técnicos.", 0.2, 'gemini-3.5-flash-lite');
            ...
            const phase5Promises = skeleton.map(async (chapter, index) => {
                ...
                try {
                    const writeRes = await this.callModel(writerSys, "Genera la matriz de alta densidad en formato JSON estricto dividiendo la acción en un mínimo de 3 a 6 beats y define la frase_bisagra_salida.", 0.2, 'gemini-3.5-flash-lite');
                    ...
                    const masterPrompt = `=== SETUP TÉCNICO === Lugar: ${setup.locacion_explicita || chapter.escenario_principal} Tiempo: ${setup.vector_tiempo || "Continuación"} Clima/Atmósfera: ${setup.clima_atmosfera || "Estándar"} Elenco: ${(setup.elenco_activo || chapter.personajes_presentes || []).join(', ')} Objetos: ${(setup.objetos_en_uso || chapter.elementos_mencionados || []).join(', ')} === LORE Y DATOS ESTRICTOS (INMUTABLES) === ${chapter.relevant_context.join('\n') || "Usar estrictamente la sinopsis sin inventar lore extra."} === DIRECTRICES DE ESCRITURA Y ENFOQUE === Tono Atmosférico: ${guidelines.tono_atmosferico || "Priorizar la descripción fidedigna."} Ritmo Narrativo: ${guidelines.ritmo_narrativo || "Fluido y natural."} === BIBLIA DE CAMBIOS Y EVOLUCIÓN === Cambio de Paradigma: ${deltas.cambio_de_paradigma || "Sin cambios estructurales."} Revelación Clave: ${deltas.revelacion_clave || "Ninguna."}`;
                    return {
