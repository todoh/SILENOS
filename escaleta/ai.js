...
const ai = {
    ...
    clientId: localStorage.getItem('pollinations_client_id') || '', // ID de cliente global (pk_...) para BYOP
    ...
    pollinationsModel: localStorage.getItem('escaleta_pollinations_model') || 'nova-fast', // Dynamic model selection
    contextHandle: null,
    inventory: [],
    ...
    login() {
        ...
        // Si hay un clientId global configurado, lo adjuntamos como parámetro
        if (this.clientId) {
            ...
            try {
                ...
                if (!response.ok) {
                    if (response.status === 402) {
                        throw new Error("CRÉDITOS INSUFICIENTES (Error 402): Tu saldo de 'Pollen' en Pollinations AI se ha agotado para procesar esta escena masiva. Por favor, recarga tu balance o cambia a Ollama.");
                    }
