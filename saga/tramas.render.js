/**
 * TRAMAS RENDER - Motor de dibujo especializado (Con soporte para burbujas de imagen redondas en el Canvas)
 */
window.TramasRender = {
    ctx: null,
    canvas: null,
    imageCache: {}, // Caché interna para evitar parpadeos y recargas de blob URLs en cada frame
    ...
    drawNode(node) {
        ...
        // --- RENDERIZADO COMPLETO DE BURBUJAS DE IMAGEN (TODOS LOS DATOS ENLAZADOS) ---
        if (node.dataRefs && node.dataRefs.length > 0) {
            const bubbleRadius = 11;
            const bubbleOverlap = 6;
            const maxRowWidth = config.nodeWidth - 24; // Espacio disponible horizontal en el nodo
            ...
            node.dataRefs.forEach((refId, index) => {
                ...
                // Cálculo de cuadrícula/salto de línea dinámico dentro del propio nodo
                ...
                const currentBubbleY = baseBubbleY - currentRow * (bubbleRadius * 2 + 4); // Sube si hay más filas

                // Dibujar silueta y borde contenedor
                this.ctx.beginPath();
                this.ctx.arc(currentBubbleX, currentBubbleY, bubbleRadius, 0, Math.PI * 2);
                this.ctx.fillStyle = '#ffffff';
                this.ctx.fill();
                this.ctx.strokeStyle = node.pov === refId ? '#eab308' : '#e5e7eb';
                ...
                if (foundItem) {
                    ...
                    if (imgUrl) {
                        if (!this.imageCache[imgUrl]) {
                            ...
                            this.ctx.drawImage(
                                loadedImg, 
                                currentBubbleX - bubbleRadius, 
                                currentBubbleY - bubbleRadius, 
                                bubbleRadius * 2, 
                                bubbleRadius * 2
                            );
                            this.ctx.restore();
    ...
    drawRegion(region) {
        ...
        this.ctx.getGlobalAlpha = state.filterMode ? 0.05 : 1.0;
