/**
 * AI IMAGE - Módulo de Generación de Imágenes (ComfyUI WebSocket Engine & Pollinations Cloud Router)
 * Dependencia: ai.core.js
 * Espacio de nombres: window.Koreh.Image
 * Versión: 4.2 - Alineación con Estándar FluxStudio de Imagenes.html
 */
...
window.Koreh.Image = {
    ...
    async generate(prompt, config = {}) {
        ...
        if (imageEngine === 'pollinations') {
            ...
            // Bucle de reintentos con tolerancia a fallos (Máximo 2 intentos)
            for (let attempt = 1; attempt <= 2; attempt++) {
                try {
                    // Limpieza y codificación estricta del prompt tal como lo hace Imagenes.html
                    const cleanPrompt = encodeURIComponent(prompt.trim());
                    
                    // Réplica exacta de la sintaxis y endpoint funcional de Imagenes.html
                    ...
                    if (attempt === 1) {
                        console.log("Servidor Pollinations inestable (500). Aplicando mitigación: Cambiando a modelo base ultra-estable 'flux' y reintentando...");
                        assignedModel = 'flux'; // Fallback de seguridad inmutable
                        await new Promise(resolve => setTimeout(resolve, 1500)); // Pequeña pausa de cortesía para desaturar la pasarela
                    } else {
                        // Si el segundo intento con el modelo de rescate también falla, lanzamos el error definitivo
                        throw new Error(`Fallo definitivo en la API de Imágenes de Pollinations tras reintento de rescate (Status: 500).`);
                    }
                }
            }
        }
