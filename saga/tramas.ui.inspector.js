...
// INSPECTOR OMEGA Y GESTIÓN DE PROPIEDADES DE NODOS (Arquitectura Blindada con Selector en Ventana Emergente Fixed)
...
window.TramasUI.openInspector = function(nodeId) {
    ...
    content.innerHTML = `
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4 pt-2">
                ...
                <div>
                    ...
                    <input type="text" class="config-input text-xs" placeholder="Ej. 3 meses / 2 hours..." value="${node.duration || ''}" oninput="TramasUI.updateNodeField('duration', this.value)">
                </div>
            </div>
        ...
        <div class="mt-6 border-t border-gray-100 pt-4">
            ...
            <button onclick="TramasUI.toggleGridSelector()" class="btn-primary py-2 text-xs font-bold tracking-wider bg-indigo-50 border border-indigo-200 text-indigo-700 hover:bg-indigo-100 w-full flex items-center justify-center gap-2 mb-3">
                <i class="fa-solid fa-grip"></i> SELECCIONAR DE LA BASE DE DATOS
            </button>

            <div id="grid-data-selector-popup" class="hidden fixed inset-0 bg-black/40 flex items-center justify-center z-[9999] p-4 backdrop-blur-sm transition-opacity">
                <div class="bg-white border border-gray-200 shadow-2xl p-6 w-full max-w-xl max-h-[85vh] flex flex-col gap-4 rounded-sm" onclick="event.stopPropagation()">
                    <div class="flex justify-between items-center border-b border-gray-100 pb-2">
                        <span class="label-text text-black mb-0 font-bold tracking-widest"><i class="fa-solid fa-cube mr-1"></i> REPOSITORIO DE ELEMENTOS</span>
                        <button onclick="TramasUI.toggleGridSelector()" class="text-gray-400 hover:text-black text-lg font-light">&times;</button>
                    </div>
                    <div class="grid grid-cols-4 gap-4 p-1 overflow-y-auto flex-1" id="grid-selector-slots">
                        </div>
                    <div class="flex justify-end pt-3 border-t border-gray-50">
                        <button onclick="TramasUI.toggleGridSelector()" class="btn-primary w-auto px-6 py-2 bg-black hover:opacity-90">CERRAR REPOSITORIO</button>
                    </div>
                </div>
            </div>
...
window.TramasUI.toggleGridSelector = function() {
    const popup = document.getElementById('grid-data-selector-popup');
    if (!popup) return;
    
    popup.classList.toggle('hidden');
    if (!popup.classList.contains('hidden')) {
        this.renderGridSelectorElements();
    }
};

window.TramasUI.renderGridSelectorElements = function() {
    const container = document.getElementById('grid-selector-slots');
    if (!container) return;
    container.innerHTML = '';
    ...
    if (window.app.items.length === 0) {
        container.innerHTML = `<div class="col-span-full text-center text-gray-400 text-xs italic py-12">No hay datos en la carpeta del repositorio.</div>`;
        return;
    }

    window.app.items.forEach(item => {
        const isAlreadyLinked = node.dataRefs && node.dataRefs.includes(item.name);
        const name = item.data.name || "Sin Nombre";
        const imageSrc = item.displayImage || item.data.imagen64;
        
        const card = document.createElement('div');
        card.className = `flex flex-col items-center justify-center p-3 border transition-all cursor-pointer text-center group rounded-md h-[110px] relative ${
            isAlreadyLinked ? 'border-indigo-500 bg-indigo-50/40 shadow-sm' : 'border-gray-100 hover:border-black bg-white hover:shadow-md'
        }`;
        
        let visualBox = `<div class="w-14 h-14 rounded-full bg-gray-50 flex items-center justify-center border border-gray-100 overflow-hidden shrink-0 transition-transform group-hover:scale-105">
            <i class="fa-solid fa-cube text-gray-300 text-sm"></i>
        </div>`;
        
        if (imageSrc) {
            visualBox = `<div class="w-14 h-14 rounded-full border border-gray-100 overflow-hidden shrink-0 bg-white transition-transform group-hover:scale-105">
                <img src="${imageSrc}" class="w-full h-full object-cover">
            </div>`;
        }

        // Checkmark visual si ya está enlazado
        const checkBadge = isAlreadyLinked ? `<div class="absolute top-1 right-1 w-4 h-4 rounded-full bg-indigo-600 text-white flex items-center justify-center text-[8px] font-bold"><i class="fa-solid fa-check"></i></div>` : '';

        card.innerHTML = `
            ${checkBadge}
            ${visualBox}
            <span class="text-[10px] font-medium text-gray-800 line-clamp-2 leading-tight mt-2 w-full tracking-tight font-sans">${name}</span>
        `;

        card.onclick = () => {
            if (!node.dataRefs) node.dataRefs = [];
            if (!node.dataRefs.includes(item.name)) {
                node.dataRefs.push(item.name);
                this.saveCurrentOmega(); 
                this.openInspector(this.selectedNodeId);
                // Mantenemos el popup modal fixed abierto para permitir flujos ágiles de clics
                const newPopup = document.getElementById('grid-data-selector-popup');
                if (newPopup) newPopup.classList.remove('hidden');
                TramasUI.renderGridSelectorElements();
            } else {
                node.dataRefs = node.dataRefs.filter(id => id !== item.name);
                if (node.pov === item.name) node.pov = null;
                this.saveCurrentOmega();
                this.openInspector(this.selectedNodeId);
                const newPopup = document.getElementById('grid-data-selector-popup');
                if (newPopup) newPopup.classList.remove('hidden');
...
window.TramasUI.closeInspector = function() {
    ...
    const sidebar = document.getElementById('omega-sidebar');
    if (sidebar) sidebar.classList.remove('open');
    setTimeout(() => {
        const actions = document.getElementById('omega-sidebar-actions');
