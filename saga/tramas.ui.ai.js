...
Object.assign(window.TramasUI, {
    ...
    async confirmExpandNode() {
        ...
        ui.setLoading(true, "Expandiendo rama de trama...");
                ...
                "eventos": [
                    {
                        ...
                        "tension": número del 1 al 5 indicando el level de tensión narrativa,
    ...
    async generatePlotFromScratch() {
        ...
        const prompt = `
            ...
            Integra elements del contexto en la trama de forma orgánica.
        ...
        try {
            const result = await window.Koreh.Text.generate(
                'Eres un arquitecto de guion. Devuelve estrictamente JSON.', 
                prompt, 
                { jsonMode: true }
            );
    ...
    async enrichMultipleNodes(nodeIds) {
        for (let i = 0; i < nodeIds.length; i++) {
            ui.setLoading(true, `IA Enriqueciendo evento ${i+1}/${nodeIds.length}...`);
            await this.enrichEventAgent(nodeIds[i], true);
        }
        
        ui.setLoading(false);
        ui.alert(`¡Proceso finalizado! Se han inyectado y enriquecido ${nodeIds.length} eventos.`);
    ...
    async enrichEventAgent(nodeId, isSilent = false) {
        ...
        try {
            ...
            if (result.dataRefs && Array.isArray(result.dataRefs)) {
                ...
                node.dataRefs = [...new Set([...(node.dataRefs || []), ...safeRefs])]; 
            }
            ...
            if (!isSilent && window.TramasCanvas && typeof window.TramasCanvas.render === 'function') {
                window.TramasCanvas.render();
            }
