// export-runtime-movement.js - SUBMÓDULO DEL MOTOR DE MOVIMIENTO, PATHFINDING Y HOOK DE SCRIPTING
function buildExportRuntimeMovement() {
    return `
        class MovementEngine {
            ...
            init() {
                ...
                this.player = scene.elements.find(e => e.isPlayer) || scene.elements.find(e => e.type === 'entidad');
                this.startLoop();
            }
            stop() {
                if (this.animFrameId) cancelAnimationFrame(this.animFrameId);
            ...
            setupHoldClickListeners() {
                window.addEventListener('mousedown', (e) => {
                    ...
                    if (e.target.closest('#game-ui') || e.target.closest('#inventory-bar') || e.target.closest('#dialog-box') || e.target.closest('#view-toggle-btn') || e.target.closest('#btn-quick-save') || e.target.closest('#main-game-menu')) {
                        return;
                    }
            ...
            checkCollisionIgnoringEntity(newX, newY, elem = this.player, ignoreEntityId = null) {
                ...
                const bg = scene.elements.find(e => e.type === 'fondo');
                const mapWidth = bg ? bg.width : dim.width;
                const mapHeight = bg ? bg.height : dim.height;
                                
                if (box.x < 0 || box.y < 0 || (box.x + box.w) > mapWidth || (box.y + box.h) > mapHeight) {
                    return true;
                }
                ...
                return scene.elements.some(other => {
                    ...
                    if (typeof checkCondition === 'function' && !checkCondition(other.condition)) return false;
            ...
            findPathAStar(startX, startY, targetX, targetY, elem = this.player) {
                const scene = projectData.scenes[currentSceneId];
                const bg = scene ? scene.elements.find(e => e.type === 'fondo') : null;
                const dim = typeof getStageDimensions === 'function' ? getStageDimensions() : { width: 960, height: 540 };
                const mapWidth = bg ? bg.width : dim.width;
                const mapHeight = bg ? bg.height : dim.height;
                                
                const cols = Math.floor(mapWidth / this.gridSize);
                const rows = Math.floor(mapHeight / this.gridSize);
            ...
            updateKeyboardMovement() {
                ...
                if (this.keysPressed['w'] || this.keysPressed['arrowup']) dirY -= 1;
                if (this.keysPressed['s'] || this.keysPressed['arrowdown']) dirY += 1;
                if (this.keysPressed['a'] || this.keysPressed['arrowleft']) dirX -= 1;
                if (this.keysPressed['d'] || this.keysPressed['arrowright']) dirX += 1;
                ...
                const stepX = normX * moveSpeed;
                const stepY = normY * moveSpeed;

                let nextX = this.player.x + stepX;
                let nextY = this.player.y + stepY;

                if (!this.checkCollision(nextX, nextY, this.player)) {
                    this.player.x = nextX;
                    this.player.y = nextY;
                } else if (!this.checkCollision(nextX, this.player.y, this.player)) {
                    this.player.x = nextX;
                } else if (!this.checkCollision(this.player.x, nextY, this.player)) {
                    this.player.y = nextY;
                }
            ...
            startLoop() {
                const update = () => {
                    ...
                    // EVALUACIÓN DE LIFECYCLE ONUPDATE(SELF, DT) EN CADA FRAME
                    const scene = projectData.scenes[currentSceneId];
                    if (scene && scene.elements) {
                        scene.elements.forEach(elem => {
                            if (typeof elementScriptRuntime !== 'undefined') {
                                elementScriptRuntime.runUpdate(elem, dt);
            ...
            updateEntitiesMovement() {
                ...
                scene.elements.forEach(elem => {
                    if (elem.isPlayer || elem.type !== 'entidad') return;
                    if (typeof checkCondition === 'function' && !checkCondition(elem.condition)) return;
            ...
            checkProximityTriggers() {
                ...
                if (dist <= effectiveDistance) {
                    ...
                    this.isMoving = false;
                    this.currentSpeed = 0;
                    if (typeof handleEntityInteraction === 'function') {
            ...
            checkPassiveTriggers() {
                ...
                const pBox = this.getColliderBox(this.player);
                scene.elements.forEach(elem => {
                    if (elem.type === 'entidad' && elem.triggerType === 'passive') {
                        if (typeof checkCondition === 'function' && !checkCondition(elem.condition)) return;
            ...
            syncElementDOM(elem) {
                ...
                if (typeof isIsometricView !== 'undefined' && isIsometricView) {
                    ...
                    } else if (billboardMode === 'cross_x' || billboardMode === 'fixed') {
