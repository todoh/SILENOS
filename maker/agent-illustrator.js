...
class AgentIllustrator {
    async generateSingleSVG(promptText, stylePrompt, apiKey, isEntity = false, abortController = null) {
        let fullPrompt = "";
        if (isEntity) {
            fullPrompt = `Asset gráfico 2D interactivo: ${promptText}. Estilo: ${stylePrompt}. Fondo transparente, totalmente aislado. El asset debe verse de frente o perspectiva top-down 2D según corresponda para ser un elemento interactivo en el mundo. Si el asset es un personaje, NPC o criatura, DEBE tener proporciones anatómicas realistas (estilizado maduro de 7-8 cabezas de alto, SIN proporciones chibi ni chubi, SIN cabeza gigante ni extremidades deformadas de estilo dibujo animado infantil).`;
        } else {
