...
async function handleDatosStudioTool(args) {
    ...
    try {
        if (action === 'create_data') {
            ...
            const newFilename = `${cleanName}_${Date.now()}.json`;
        ...
        // NUEVA ACCIÓN: ILUSTRAR CON SVG NATIVO CREADO POR GEMINI
            ...
            // Convertimos el SVG puro a un Base64 DataURI robusto 
            const base64Svg = btoa(unescape(encodeURIComponent(svg_content)));
            data.imagen64 = `data:image/svg+xml;base64,${base64Svg}`;
