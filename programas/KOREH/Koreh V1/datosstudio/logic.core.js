...
class DataStudio {
    ...
    init() {
        ...
        if(FS.rootHandle) this.rootHandle = FS.rootHandle;
    ...
    async loadFiles() {
        ...
        try {
            for await (const entry of this.targetHandle.values()) {
                // FILTRO: Ignoramos TIMELINE y RESUMEN_GLOBAL para que no salgan en la grid
                if (entry.kind === 'file' && entry.name.endsWith('.json') && !entry.name.includes('TIMELINE') && !entry.name.includes('RESUMEN_GLOBAL')) {
