NodeRegistry.register('ai-image', {
    label: 'GENERADOR IMAGEN',
    inputs: ['in_1'],
    outputs: ['out_1'],
    defaults: { model: "flux", ratio: "landscape", chroma: false },
    ...
    execute: async (node, inputData) => {
        ...
        let imgBlob = await window.AIImage.generate(inputData, node.data);
        
        if (node.data.chroma === true) {
            window.app.log("Aplicando Chroma Key...", "info");
            imgBlob = await window.ChromaProcessor.process(imgBlob);
        }
        return imgBlob;
    }
});