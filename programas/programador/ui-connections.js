...
Object.assign(window.UI, {
    ...
    renderConnections(tempX, tempY) {
        ...
        Logic.connections.forEach(conn => {
            // CORRECCIÓN: Usamos data-port para seleccionar el puerto ESPECÍFICO, no solo el primero que encuentre
            const fromEl = document.querySelector(`#${conn.from} .port-out[data-port="${conn.fromPort}"]`);
            const toEl = document.querySelector(`#${conn.to} .port-in[data-port="${conn.toPort}"]`);
