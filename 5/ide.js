...
let ideAutoSaveTimer = null; // Temporizador global para Auto-guardado en persistencia
...
const ide = {
    ...
    open() {
        ...
        // Enlazamos el Auto-guardado de texto a persistencia absoluta
        const editor = document.getElementById('ideEditor');
        editor.removeEventListener('input', this._handleInput);
        editor.addEventListener('input', this._handleInput.bind(this));
    },
    ...
    _handleInput() {
        clearTimeout(ideAutoSaveTimer);
        ideAutoSaveTimer = setTimeout(() => {
            this.saveFile(true); // Guardado silencioso de persistencia
        }, 1000);
    },
    ...
    async saveFile(isAutoSave = false) {
        ...
        try {
            const newContent = document.getElementById('ideEditor').value;

            // Inyectar en el Registro / UndoStack SOLO si es un guardado manual forzado para evitar saturar el historial en la persistencia dinámica
            if (!isAutoSave) {
                ...
                if (oldContent !== newContent) {
                    ...
                    historyManager.logAction('USUARIO (IDE)', `Forzó el guardado en disco de ${this.currentFileHandle.name}`);
                }
            }
            ...
            if (!isAutoSave && typeof showToast === 'function') showToast('Archivo guardado en histórico ✓', 'success');
        } catch (e) {
    ...
    async createFile() {
        ...
        try {
            ...
            historyManager.logAction('USUARIO (IDE)', `Creó archivo físico: ${fullPath}`);
            
            this.refreshTree(); // Actualiza IDE
            if(typeof visualizadorUI !== 'undefined') visualizadorUI.renderCurrentFolder(); // Sincronización cruzada
    ...
    async createFolder() {
        ...
        try {
            ...
            historyManager.logAction('USUARIO (IDE)', `Creó carpeta física: ${fullPath}`);
    ...
    async paste() {
        ...
        try {
            if (this.clipboard.isDir) {
                ...
                historyManager.logAction('USUARIO (IDE)', `Pegó estructura de carpeta: ${this.clipboard.name}`);
            } else {
                ...
                await writable.close();
                historyManager.logAction('USUARIO (IDE)', `Pegó archivo: ${newName}`);
            }
            
            if (this.currentDirPath) this.expandedPaths.add(this.currentDirPath);
            this.refreshTree();
            if(typeof visualizadorUI !== 'undefined') visualizadorUI.renderCurrentFolder();
    ...
    async createBackup(name, isDir, event, fullPath) {
        ...
        try {
            ...
            historyManager.logAction('USUARIO (IDE)', `Generó Backup de contingencia: ${name}`);
    ...
    async deleteEntry(name, isDir, event, fullPath) {
        ...
        try {
            ...
            historyManager.logAction('USUARIO (IDE)', `Purgó el elemento: ${name}`);
