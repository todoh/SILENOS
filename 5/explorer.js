...
const explorerLens = {
    async getHandleFromPath(path, baseHandle = null) {
        ...
        if (!root) throw new Error("No hay carpeta seleccionada por el usuario.");
    ...
    async analyzeTarget(path, model = 'gemini-1.5-flash') {
        return `=== ANÁLISIS DESACTIVADO PARA '${path}' ===\nLas llamadas REST en segundo plano están desactivadas. Solo opera la voz de Gemini Live.`;
    }
};